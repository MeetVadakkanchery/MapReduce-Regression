...
public class ProductRegressionReducer extends Reducer<Text, Text, Text, Text> {
	...
	public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
		...
		while(initial < 3000000){
			...
			double mean = sum_y / sum_1;
			double delta  = (result - mean)*100/(mean);
			context.write(key, new Text(","+Double.toString(initial)+","+Long.toString((long)result)+", "+Double.toString(delta)));
			initial*=1.05;
