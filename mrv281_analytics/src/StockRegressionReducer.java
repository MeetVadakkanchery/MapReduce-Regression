...
public class StockRegressionReducer extends Reducer<Text, Text, Text, Text> {
	...
	public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
		...
		for(Text val:values){
			...
			long y = Long.parseLong(data[0]);
			long x = (long)Double.parseDouble(data[1]);
			sum_x += x;
			sum_x_sq += (x*x);
			sum_y_sq += (y*y);
			sum_y += y;
		...
		double inital = 3000;
		while (inital <= 4500){
			double result = reg.getPrediction(inital);
			context.write(new Text(Double.toString(inital)), new Text(Long.toString((long)result)));
			inital*=1.05;
			inital = Math.floor(inital);
		}
